...
### Option 1 — Standalone Windows executable (recommended, no Python required)

1. Download **`gameplay.zip`** from the [repository root](https://github.com/gimmeurcode/netexecutive). It includes the full build and `NETEXEC.exe` (Windows) + `NETEXEC.app` (macOS).
2. Extract the zip — you get a `gameplay\` folder.
3. Run **`gameplay\NETEXEC.exe`** and play.

Settings are saved automatically to `%APPDATA%\NETEXEC\settings.json`. The exe is a single self-contained file — no installer, no dependencies.

---

### Option 2 — Standalone macOS app (no Python required)

1. Download **`gameplay.zip`** from the [repository root](https://github.com/gimmeurcode/netexecutive).
2. Extract the zip — you get a `gameplay/` folder.
3. Open **`gameplay/NETEXEC.app`** (right-click → **Open** the first time).

Settings are saved automatically to `~/.config/NETEXEC/settings.json`. The app bundle is self-contained — no installer, no dependencies.
...
### Option 4 — Rebuild the standalone build

Produces a fresh standalone build from source. Requires Python 3.11+ (PyInstaller is auto-installed).

```bash
python dev/build_game.py
```

Output: `gameplay/NETEXEC.exe` (Windows) or `gameplay/NETEXEC.app` (macOS) — no Python needed to run.
...
gameplay/
│
├── NETEXEC.exe          Standalone Windows executable (no Python needed).
├── NETEXEC.app          Standalone macOS app bundle (no Python needed).
├── main.py              Entry point — pygame init, game loop.
├── run_game.bat         Windows double-click launcher (Python runner).
├── run_game.sh          macOS / Linux double-click launcher.
│
├── scripts/
...
│
├── core/                Shim re-exports from scripts/engine/ (UI compat layer).
├── pools/               Shim re-exports from scripts/content/ (UI compat layer).
│
├── ui/
...
│   └── screens/         9 per-screen modules.
│
├── assets/              Design-system SVGs, fonts, CSS.
│
└── data/
    ├── shows.json        47 shows across 6 genres + wildcard template.
    ├── stars.json        36 stars with JSON-encoded conditions.
    ├── ads.json          30 ads including wildcard template.
    ├── upgrades.json     26 global upgrades.
    ├── events.json       18 one-off events.
...
_Generated from data files. Re-run `python dev/tests/devscripts/gen_compendium.py` after any content change._
...
|---|---|
| **Sitcom** | ×1.15 views & +$10 income/season per show |
| **Drama** | ×1.22 views & +$18 income/season per show |
| **Scifi** | ×1.25 views & +$14 income/season per show |
| **Reality** | ×1.1 views & +$10 income/season per show |
| **Sports** | ×1.3 views & +$20 income/season per show |
| **News** | Milestone target ×0.88 & +$5 income/season |
| **Cooking** | ×1.18 views & +$12 income/season per show |
...
| Production Tax Incentive | $8 | -$3 upkeep from all shows |
