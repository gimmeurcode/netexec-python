...
def calculate_yield(show: dict, *,
    ...
    if not show or show.get("is_extension"):
        return {"v": 0, "i": 0, "ad_income": 0, "star_income": 0, "upkeep": 0, "slot_indices": []}
    ...
    # 5b. Wildcard show ability (live shows only; no-op when field absent)
    if not is_rerun:
        wa = show.get("wildcard_ability_effect") or {}
        v      += wa.get("v_flat",  0)
        mult   *= wa.get("v_mult",  1.0)
        income += wa.get("income",  0)
        upkeep += wa.get("upkeep",  0)

    mult, income = _stage_monopoly(mult, income, monopoly_bonus, is_rerun)  # 6. monopoly

    mult, income, upkeep = _stage_seasonal(                         # 6b. seasonal
