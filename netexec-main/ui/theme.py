...
Usage
-----
UI code that only needs the C_* palette can keep importing from scripts.engine.constants.
...
def load_fonts(assets_dir: str | os.PathLike | None = None) -> dict[str, pygame.font.Font]:
    """
    Return a dict of pygame font objects keyed by role name.

    Tries TTF files in assets_dir/fonts/ first (Saira + IBM Plex Mono from the
    design spec), then falls back to the preferred system monospace chain.
    The fonts/ directory is intentionally empty until licensed faces are added;
    the fallback produces identical rendering to the pre-theme build.
    """
    if assets_dir is None:
        assets_dir = pathlib.Path(__file__).parent.parent / "assets" / "fonts"
    else:
    ...
    for role, (size, bold) in FONT_ROLES.items():
        ...
        f = (
            ...
            _try_ttf("IBMPlexMono", size, bold) if role not in ("title", "header") else None
        ) or _sysfont(size, bold)
        result[role] = f
    return result
