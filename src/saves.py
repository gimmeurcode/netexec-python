...
def delete_save(slot: int = 0) -> None:
    """
    Delete the save file for the given slot (called when starting a fresh run).
    ...
    try:
        os.remove(path)
        logger.info("[platform] Save deleted: %s", path)
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.warning("[platform] Failed to delete save: %s", exc)
