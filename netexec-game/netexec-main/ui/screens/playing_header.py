...
def _draw_header(ctx, state):
    ...
    ctx.screen.blit(s_surf, (cl_x, 6))

    bar_w    = cl_w - 6
    bar_rect = pygame.Rect(cl_x, 28, bar_w, 9)
    ...
    if fill_w > 0:
        ...
        pygame.draw.rect(ctx.screen, fill_col,
                         pygame.Rect(cl_x, 28, fill_w, 9), border_radius=3)
    ...
    ctx.screen.blit(v_surf, (cl_x, 41))

    quota_str = (f"QUOTA: {state.current_target // 1000}k views"
                 f"  -  NEXT S{state.next_target}")
    q_surf    = ctx._f("micro").render(quota_str, True, C_GREY_MID)
    ctx.screen.blit(q_surf, (cl_x, 58))
    ...
    badge_rect = pygame.Rect(cr_x, 52, badge_surf.get_width() + 14, 16)
    ...
    ctx.screen.blit(badge_surf, (cr_x + 7, 54))
    ...
    ctx.screen.blit(inc_surf, (rb_x, 34))
    ...
    net_surf = ctx._f("micro").render(
        f"PROJECTED END: {net_sign}${proj_net:.0f}/season", True, net_col
    )
    ctx.screen.blit(net_surf, (rb_x, 46))

    # LEDGER / SHOP toggle button (right-aligned in block 4)
    tog_label = "[SHOP]" if getattr(ctx, "_show_ledger", False) else "[LEDGER]"
    tog_w     = min(72, rb_w - 6)
    tog_rect  = pygame.Rect(rb_x + rb_w - tog_w - 2, 4, tog_w, 16)
    ...
    pause_rect = pygame.Rect(rb_x, 58, pause_w, 14)
    ...
    if has_shows:
        ...
        draw_button(ctx, air_rect, "SCHEDULE SHOWS FIRST", _air_season,
