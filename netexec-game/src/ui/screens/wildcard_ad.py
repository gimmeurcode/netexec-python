"""
wildcard_ad.py — NETEXEC
=========================
Wildcard ad configuration modal — 3-step flow:
  Step 1: enter a name
  Step 2: pick one of 3 offered target genres
...
from ..theme import C_TINT_AMBER_NOTCH, C_TINT_AMBER_SEL, C_TINT_AMBER_FALLBACK
...
def render(ctx, state):
    ...
    pygame.draw.rect(ctx.screen, C_AMBER, modal, 2, border_radius=8)

    step = ctx._wc_step
    step_labels = [
        "STEP 1 OF 3: NAME YOUR AD",
        "STEP 2 OF 3: CHOOSE TARGET GENRE",
        "STEP 3 OF 3: CHOOSE ABILITY",
    ]
    title = ctx._f("header").render(
        f"WILDCARD AD - {step_labels[step - 1]}", True, C_AMBER)
...
def _step1(ctx, state, modal, cx, y):
    draw_label(ctx, "AD NAME  (click field, then type):", modal.x + 16, y)
    y += 20
    nr = pygame.Rect(modal.x + 16, y, modal.width - 32, 30)
    pygame.draw.rect(ctx.screen, C_TINT_AMBER_NOTCH, nr)
    pygame.draw.rect(ctx.screen, C_AMBER if ctx._wc_input_active else C_BORDER, nr, 1)
    ...
    draw_button(ctx, next_rect, "NEXT ►", _next, border_color=C_AMBER)
    _cancel_btn(ctx, state, modal.right - 126, btn_y)


# ─── STEP 2: TARGET GENRE SELECTION ──────────────────────────────────────────

def _step2(ctx, state, modal, cx, y):
    name_disp = ctx._wc_name or "???"
    sub = ctx._f("small").render(f'Ad: "{name_disp}"', True, C_GREEN_DIM)
    ctx.screen.blit(sub, sub.get_rect(center=(cx, y)))
    y += 28

    draw_label(ctx, "SELECT TARGET GENRE:", modal.x + 16, y)
    y += 22

    note = ctx._f("small").render(
        "Match income applies when the ad is placed on a show of this genre.", True, C_GREY_MID)
    ...
    if types:
        ...
        for i, gid in enumerate(types):
            ...
            gcol = GENRE_COLORS.get(gid, (C_AMBER, C_TINT_AMBER_FALLBACK))
...
def _step3(ctx, state, modal, cx, y):
    ...
    sub = ctx._f("small").render(
        f'Ad: "{name_disp}"  -  Target: {ctx._wc_genre}', True, C_GREEN_DIM)
    ctx.screen.blit(sub, sub.get_rect(center=(cx, y)))
    y += 28
    ...
    for ab in (ctx._wc_offered_abilities or []):
        ...
        pygame.draw.rect(ctx.screen, C_TINT_AMBER_SEL if sel else C_BG, card, border_radius=5)
        pygame.draw.rect(ctx.screen, C_AMBER if sel else C_BORDER,
        ...
        eff_surf = ctx._f("small").render(_fmt_effect(eff), True, C_AMBER)
    ...
    if ctx._wc_chosen_ability:
        def _confirm():
            result = state.resolve_wildcard_ad(
        ...
        draw_button(ctx, pygame.Rect(cx - 70, btn_y, 140, 38),
                    "CONFIRM ►", _confirm, border_color=C_AMBER)
